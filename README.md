...
|---|---|---|---|---|
| adduser_conf_template | /etc/adduser.conf template location. | path | no | etc/adduser.conf.j2 |
...
| ipv6_disable_sysctl_settings.net.ipv6.conf.all.disable_ipv6 | If 1, the system disables IPv6 on all interfaces. | int | no | 1 |
| ipv6_disable_sysctl_settings.net.ipv6.conf.default.disable_ipv6 | If 1, the system disables IPv6 on the default interface. | int | no | 1 |
| ipv6_sysctl_settings.net.ipv6.conf.all.accept_ra | If 0, the system does not accept IPv6 router advertisements. | int | no | 0 |
| ipv6_sysctl_settings.net.ipv6.conf.all.accept_redirects | If 0, the system does not accept IPv6 redirects. | int | no | 0 |
| ipv6_sysctl_settings.net.ipv6.conf.all.accept_source_route | If 0, deny forwarding of IPv6 Source Routed Packets | int | no | 0 |
| ipv6_sysctl_settings.net.ipv6.conf.all.forwarding | If 0, denies IPv6 forwarding. | int | no | 0 |
| ipv6_sysctl_settings.net.ipv6.conf.all.use_tempaddr | Preference for IPv6 Privacy Extensions on all interfaces. | int | no | 2 |
| ipv6_sysctl_settings.net.ipv6.conf.default.accept_ra | If 0, the system does not accept IPv6 router advertisements on the default interface. | int | no | 0 |
| ipv6_sysctl_settings.net.ipv6.conf.default.accept_ra_defrtr | If 0, the system does not accept default routers from IPv6 router advertisements on the default interface. | int | no | 0 |
| ipv6_sysctl_settings.net.ipv6.conf.default.accept_ra_pinfo | If 0, the system does not accept prefix information from IPv6 router advertisements on the default interface. | int | no | 0 |
| ipv6_sysctl_settings.net.ipv6.conf.default.accept_ra_rtr_pref | If 0, the system does not accept router preference from IPv6 router advertisments on the default interface. | int | no | 0 |
| ipv6_sysctl_settings.net.ipv6.conf.default.accept_redirects | If 0, the system does not accept IPv6 redirects on the default interface. | int | no | 0 |
| ipv6_sysctl_settings.net.ipv6.conf.default.accept_source_route | If 0, deny forwarding of IPv6 Source Routed Packets on the default interface. | int | no | 0 |
| ipv6_sysctl_settings.net.ipv6.conf.default.autoconf | If 0, the system doesn't autoconfigure addresses using Prefix Information in Router Advertisements. | int | no | 0 |
| ipv6_sysctl_settings.net.ipv6.conf.default.dad_transmits | The amount of Duplicate Address Detection probes to send. | int | no | 0 |
| ipv6_sysctl_settings.net.ipv6.conf.default.max_addresses | Maximum number of autoconfigured IPv6 addresses per interface. | int | no | 1 |
| ipv6_sysctl_settings.net.ipv6.conf.default.router_solicitations | Number of Router Solicitations to send until assuming no routers are present. | int | no | 0 |
| ipv6_sysctl_settings.net.ipv6.conf.default.use_tempaddr | Preference for IPv6 Privacy Extensions on the default interface. | int | no | 2 |
...
| apparmor_sysctl_settings.kernel.apparmor_display_secid_mode | If 1, AppArmor will provide a human-readable mapping of internal security IDs. | int | no | 0 |
| apparmor_sysctl_settings.kernel.apparmor_restrict_unprivileged_io_uring | If 1, restrict unprivileged io_uring operations. | int | no | 0 |
| apparmor_sysctl_settings.kernel.apparmor_restrict_unprivileged_unconfined | If 1, enforces extra security restrictions on unprivileged unconfined processes. | int | no | 1 |
| apparmor_sysctl_settings.kernel.apparmor_restrict_unprivileged_userns | If 1, restricts unprivileged user namespaces. | int | no | 1 |
| apparmor_sysctl_settings.kernel.apparmor_restrict_unprivileged_userns_complain | If 1, unprivileged user namespaces will be put into complain mode. | int | no | 0 |
| apparmor_sysctl_settings.kernel.apparmor_restrict_unprivileged_userns_force | If 1, all confined applications will have the user namespace mediation enforced. | int | no | 0 |
| apparmor_sysctl_settings.kernel.unprivileged_userns_apparmor_policy | If 1, unprivileged user namespaces will be restricted to the AppArmor policy. | int | no | 1 |
| conntrack_sysctl_settings.net.netfilter.nf_conntrack_max | The maximum number of connections that the system’s connection tracking table can hold at any time. | int | no | 2000000 |
| conntrack_sysctl_settings.net.netfilter.nf_conntrack_tcp_loose | If 0, the system will not allow TCP connections to be established without a SYN packet. | int | no | 0 |
| generic_sysctl_settings.fs.protected_fifos | Avoid unintentional writes to an attacker-controlled FIFO, where a program expected to create a regular file. | int | no | 2 |
| generic_sysctl_settings.fs.protected_regular | Avoid unintentional writes to an attacker-controlled regular file, where a program expected to create one. | int | no | 2 |
| generic_sysctl_settings.fs.protected_hardlinks | Restrict hard links to files that are not owned by the user. | int | no | 1 |
| generic_sysctl_settings.fs.protected_symlinks | Restrict symbolic links to files that are not owned by the user. | int | no | 1 |
| generic_sysctl_settings.fs.suid_dumpable | Set the core dump mode for setuid or otherwise protected/tainted binaries. | int | no | 0 |
| generic_sysctl_settings.kernel.core_pattern | Specify a core dumpfile pattern name. | str | no | |/bin/false |
| generic_sysctl_settings.kernel.core_uses_pid | If 1, the core dump file name will include the PID of the process that created it. | int | no | 1 |
| generic_sysctl_settings.kernel.dmesg_restrict | If 1, unprivileged users are prevented from using dmesg to view messages from the kernel’s log buffer. | int | no | 1 |
| generic_sysctl_settings.kernel.kptr_restrict | Whether restrictions are placed on exposing kernel addresses via /proc and other interfaces. | int | no | 2 |
| generic_sysctl_settings.kernel.panic | The value in this file determines the behaviour of the kernel on a panic. | int | no | 60 |
| generic_sysctl_settings.kernel.panic_on_oops | If 1, Panic immediately. If the panic sysctl is also non-zero then the machine will be rebooted. | int | no | 1 |
...
| auditd_disk_error_action | auditd_disk_error_action description | str | no | suspend |
| auditd_disk_full_action | auditd_disk_full_action description | str | no | suspend |
| auditd_enable_flag | auditd_enable_flag description | int | no | 2 |
| auditd_flush | auditd_flush description | str | no | incremental_async |
| auditd_max_log_file | auditd_max_log_file description | int | no | 20 |
| auditd_max_log_file_action | auditd_max_log_file_action description | str | no | rotate |
| auditd_mode | auditd_mode description | int | no | 1 |
| auditd_num_logs | auditd_num_logs description | int | no | 5 |
| auditd_space_left | auditd_space_left description | int | no | 75 |
| auditd_space_left_action | auditd_space_left_action description | str | no | email |
| grub_audit_backlog_cmdline | grub_audit_backlog_cmdline description | str | no | audit_backlog_limit=8192 |
| grub_audit_cmdline | grub_audit_cmdline description | str | no | audit=1 |
| manage_systemd | manage_systemd description | bool | no | True |
| session_timeout | session_timeout description | int | no | 900 |
| umask_value | umask_value description | str | no | 077 |
| manage_kernel | manage_kernel description | bool | no | True |
| allow_virtual_system_calls | allow_virtual_system_calls description | bool | no | True |
| enable_page_poisoning | enable_page_poisoning description | bool | no | True |
